...
namespace gpopt
{
	...
	class CUtils
	{
		...
		public:
			...
			static
			void ValidateCTEProducerConsumerLocality(IMemoryPool *memory_pool, CExpression *pexpr, EExecLocalityType edt, HMUlUl *phmulul);
			...
			static
			CExpression *PexprCmpWithZero(IMemoryPool *memory_pool, CExpression *pexprLeft, IMDId *pmdidTypeLeft, IMDType::ECmpType ecmptype);
			...
			static
			CExpression *PexprScalarArrayCmp(IMemoryPool *memory_pool, CScalarArrayCmp::EArrCmpType earrcmptype, IMDType::ECmpType ecmptype, DrgPexpr *pexprScalarChildren, const CColRef *pcr);
			...
			static
			CExpression *PexprScalarIdent(IMemoryPool *memory_pool, const CColRef *pcr);

			// generate a scalar project element expression
			static
			CExpression *PexprScalarProjectElement(IMemoryPool *memory_pool, CColRef *pcr, CExpression *pexpr);
			...
			static
			CExpression *PexprCountStarAndSum(IMemoryPool *memory_pool, const CColRef *pcr, CExpression *pexprLogical);

			// generate a sum(col) expression
			static
			CExpression *PexprSum(IMemoryPool *memory_pool, const CColRef *pcr);
			...
			static
			CExpression *PexprScalarConstOid(IMemoryPool *memory_pool, OID oidVal);
			...
			static
			DrgPcr *PdrgpcrRedistributableSubset(IMemoryPool *memory_pool, DrgPcr *pdrgpcr);
			...
			static
			BOOL FFunctionallyDependent(IMemoryPool *memory_pool, CDrvdPropRelational *pdprel, CColRefSet *pcrsKey, CColRef *pcr);

			// construct an array of colids from the given array of column references
			static
			ULongPtrArray *Pdrgpul(IMemoryPool *memory_pool, const DrgPcr *pdrgpcr);
			...
			static
			CColRefSet *PcrsRemap(IMemoryPool *memory_pool, CColRefSet *pcrs, HMUlCr *phmulcr, BOOL fMustExist);

			// create an array of column references corresponding to the given array
			// and based on the given mapping
			static
			DrgPcr *PdrgpcrRemap(IMemoryPool *memory_pool, DrgPcr *pdrgpcr, HMUlCr *phmulcr, BOOL fMustExist);

			// create an array of column references corresponding to the given array
			// and based on the given mapping and create new colrefs if necessary
			static
			DrgPcr *PdrgpcrRemapAndCreate(IMemoryPool *memory_pool, DrgPcr *pdrgpcr, HMUlCr *phmulcr);

			// create an array of column arrays corresponding to the given array
			// and based on the given mapping
			static
			DrgDrgPcr *PdrgpdrgpcrRemap(IMemoryPool *memory_pool, DrgDrgPcr *pdrgpdrgpcr, HMUlCr *phmulcr, BOOL fMustExist);

			// remap given array of expressions with provided column mappings
			static
			DrgPexpr *PdrgpexprRemap(IMemoryPool *memory_pool, DrgPexpr *pdrgpexpr, HMUlCr *phmulcr);
			
			// create ColRef->ColRef mapping using the given ColRef arrays
			static
			HMUlCr *PhmulcrMapping(IMemoryPool *memory_pool, DrgPcr *pdrgpcrFrom, DrgPcr *pdrgpcrTo);

			// add col ID->ColRef mappings to the given hashmap based on the
			// given ColRef arrays
			static
			void AddColumnMapping(IMemoryPool *memory_pool, HMUlCr *phmulcr, DrgPcr *pdrgpcrFrom, DrgPcr *pdrgpcrTo);

			// create a copy of the array of column references
			static
			DrgPcr *PdrgpcrExactCopy(IMemoryPool *memory_pool, DrgPcr *pdrgpcr);
			...
			static
			DrgPcr *PdrgpcrCopy(IMemoryPool *memory_pool, DrgPcr *pdrgpcr, BOOL fAllComputed = false, HMUlCr *phmulcr = NULL);
			...
			static
			DrgPexpr *PdrgpexprScalarIdents(IMemoryPool *memory_pool, DrgPcr *pdrgpcr);
			...
			static CExpression *PexprLimit(IMemoryPool *memory_pool, CExpression *pexpr, ULONG ulOffSet, ULONG ulCount);
		...
		)
	{
		...
		return GPOS_NEW(memory_pool) CExpression
				(
				memory_pool,
				GPOS_NEW(memory_pool) T(memory_pool, pdrgpcr, eopidOriginSubq),
				pexprLeft,
				pexprRight,
				pexprScalar
				);
