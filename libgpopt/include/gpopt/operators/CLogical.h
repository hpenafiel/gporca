...
namespace gpopt
{
	...
	class CLogical : public COperator
	{
		...
		public:
			...
			// derive join depth
			virtual
			ULONG UlJoinDepth(IMemoryPool *memory_pool, CExpressionHandle &exprhdl) const;
			...
			// derive statistics
			virtual
			IStatistics *PstatsDerive(IMemoryPool *memory_pool, CExpressionHandle &exprhdl, DrgPstat *pdrgpstatCtxt) const = 0;
			...
			virtual
			CColRefSet *PcrsStat(IMemoryPool *memory_pool, CExpressionHandle &exprhdl, CColRefSet *pcrsInput, ULONG ulChildIndex) const = 0;

			// compute partition predicate to pass down to n-th child
			virtual
			CExpression *PexprPartPred(IMemoryPool *memory_pool, CExpressionHandle &exprhdl, CExpression *pexprInput, ULONG ulChildIndex) const;
			...
			static
			CColRefSet *PcrsDist(IMemoryPool *memory_pool, const CTableDescriptor *ptabdesc, const DrgPcr *pdrgpcr);
