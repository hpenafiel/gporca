	...
	)
{
	...
	ppf = GPOS_NEW(memory_pool) CPartFilter(ulScanId, pexpr, pstats);

#ifdef GPOS_DEBUG
	BOOL fSuccess =
#endif // GPOS_DEBUG
	m_phmulpf->Insert(GPOS_NEW(memory_pool) ULONG(ulScanId), ppf);
	...
	)
{
	...
	{
		ppf->AddRef();
		ULONG *pulScanId = GPOS_NEW(memory_pool) ULONG(ulScanId);
