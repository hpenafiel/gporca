	...
	)
{
	...
	if (0 < pdrgpexprPushable->Size())
	{
		...
		CExpression *pexprConstTrue = CUtils::PexprScalarConstBool(memory_pool, true /*value*/);
	...
	)
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		if (!pcrsChild->Equals(pcrsOutput))
		{
			...
			HMUlCr *phmulcr = CUtils::PhmulcrMapping(memory_pool, pdrgpcrOutput, pdrgpcrChild);
			pexprRemappedConj->Release();
			pexprRemappedConj = pexprConj->PexprCopyWithRemappedColumns(memory_pool, phmulcr, true /*fMustExist*/);
			phmulcr->Release();
		}
