	...
	)
{
	...
	CLogicalGbAgg *popAgg = GPOS_NEW(memory_pool) CLogicalGbAgg(memory_pool, pdrgpcr, popAggOriginal->Egbaggtype());
	...
	)
{
	...
	if (fSuccess)
	{
		...
		if (NULL == pexprPredicate)
		{
			pexprPredicate = CUtils::PexprScalarConstBool(memory_pool, true /*value*/);
		}
