	...
	const
{
	...
	if (NULL == exprhdl.Pdprel(0)->Pkc() ||
		0 == exprhdl.Pdprel(1)->PcrsOuter()->Size())
	{
		return ExfpNone;
	}
	...
	const
{
	...
	DrgPcr *pdrgpcr = CUtils::PdrgpcrGroupingKey(memory_pool, pexprOuter, &pdrgpcrKey);
	pdrgpcrKey->Release();  // key is not used here

	CLogicalGbAgg *popGbAgg = GPOS_NEW(memory_pool) CLogicalGbAgg(memory_pool, pdrgpcr, COperator::EgbaggtypeGlobal /*egbaggtype*/);
