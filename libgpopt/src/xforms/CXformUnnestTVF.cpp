	...
	)
{
	...
	DrgPcr *pdrgpcrProdOutput = CDrvdPropRelational::Pdprel(pexprCTEProd->PdpDerive())->PcrsOutput()->Pdrgpcr(memory_pool);
	DrgPcr *pdrgpcrConsOutput = CDrvdPropRelational::Pdprel(pexprCTECons->PdpDerive())->PcrsOutput()->Pdrgpcr(memory_pool);
	...
	const
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		if (CDrvdPropScalar::Pdpscalar(pexprScalarChild->PdpDerive())->FHasSubquery())
		{
			...
			pdrgpexprNewArgs->Append(CUtils::PexprScalarIdent(memory_pool, pcr));
			ulIndex++;
		}
		else
		{
