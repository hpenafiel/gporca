...
namespace gpos
{
	class CTaskContext
	{
		...
		public:
			...
			CTaskContext(IMemoryPool *pmp, const CTaskContext &task_ctxt);
			...
			// set trace flag
			BOOL Trace(ULONG trace, BOOL val);
			...
			CBitSet *copy_trace_flags(IMemoryPool *pmp) const
			{
				return GPOS_NEW(pmp) CBitSet(pmp, *m_bitset);
			}
