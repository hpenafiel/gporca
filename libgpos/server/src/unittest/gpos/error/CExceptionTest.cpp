...
CExceptionTest::EresUnittest_AdditionOverflow()
{
	GPOS_TRY
	{
		...
		(void) gpos::Add(ULLONG_MAX - 2, 1);
		(void) gpos::Add(ULLONG_MAX, 0);
		(void) gpos::Add(ULLONG_MAX - 2, 2);
	}
	GPOS_CATCH_EX(ex)
	{
...
CExceptionTest::EresUnittest_MultiplicationOverflow()
{
	GPOS_TRY
	{
		...
		(void) gpos::Multiply(ULLONG_MAX, 1);
		(void) gpos::Multiply(ULLONG_MAX, 0);
		(void) gpos::Multiply(ULLONG_MAX / 2, 2);
		(void) gpos::Multiply(ULLONG_MAX / 2 - 1, 2);
	}
	GPOS_CATCH_EX(ex)
	{
		// no exception is expected here
	...
	(void) gpos::Multiply(ULLONG_MAX - 4, 2);
