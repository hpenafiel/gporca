	...
	m_allow_global_new(true)
{
	...
	m_global_memory_pool = Create(EatTracker, true, ULLONG_MAX);
}
	...
	)
{
	...
	// accessor's scope
	{
		...
		while (NULL != found)
		{
			GPOS_ASSERT(found != pmp && "Attempt to delete a registered memory pool");
...
CMemoryPoolManager::TotalAllocatedSize()
{
	...
	while (iter.Advance())
	{
		...
		if (NULL != pmp)
		{
			total_size = total_size + pmp->TotalAllocatedSize();
		}
	}
	...
	)
{
	...
	while (iter.Advance())
	{
		...
		if (NULL != pmp)
		{
			ULLONG size = pmp->TotalAllocatedSize();
