	...
	)
{
	...
	else
	{
		...
		m_os << pstrElem->GetBuffer() << CDXLTokens::GetDXLTokenStr(EdxltokenBracketCloseTag)->GetBuffer(); // >
		if (m_indentation)
		{
			m_os << std::endl;
		}
	...
	)
{
	...
	if (fValue)
	{
		pstrValue = CDXLTokens::GetDXLTokenStr(EdxltokenTrue);
	}
	else
	{
		pstrValue = CDXLTokens::GetDXLTokenStr(EdxltokenFalse);
	}
