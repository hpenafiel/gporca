	...
	)
{
	...
	GPOS_ASSERT(CMDIdGPDB::m_mdidDate.Equals(popScalarLeft->Pdatum()->MDId()));
	...
	GPOS_ASSERT(CMDIdGPDB::m_mdidDate.Equals(popScalarRight->Pdatum()->MDId()));
	...
	CExpression *pexprResult = CUtils::PexprScalarConstBool(m_memory_pool, fResult);
