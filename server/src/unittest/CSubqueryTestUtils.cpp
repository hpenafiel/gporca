	...
	)
{
	...
	CTableDescriptor *ptabdescR = CTestUtils::PtabdescCreate(memory_pool, 3 /*ulCols*/, pmdidR, CName(&strNameR));
	*ppexprOuter = CTestUtils::PexprLogicalGet(memory_pool, ptabdescR, &strNameR);
	...
	CTableDescriptor *ptabdescS = CTestUtils::PtabdescCreate(memory_pool, 3 /*ulCols*/, pmdidS, CName(&strNameS));
	...
	)
{
	...
	CTableDescriptor *ptabdescT = CTestUtils::PtabdescCreate(memory_pool, 3 /*ulCols*/, pmdidT, CName(&strNameT));
	...
	)
{
	...
	CExpression *pexprSubqueryQuantified1 = PexprSubqueryQuantified(memory_pool, eopid, pexprOuter3, pexprInner, fCorrelated);
	CExpression *pexprSelect1 = CUtils::PexprLogicalSelect(memory_pool, pexprOuter3, pexprSubqueryQuantified1);
	CExpression *pexprSubqueryQuantified2 = PexprSubqueryQuantified(memory_pool, eopid, pexprOuter2, pexprSelect1, fCorrelated);
	CExpression *pexprSelect2 = CUtils::PexprLogicalSelect(memory_pool, pexprOuter2, pexprSubqueryQuantified2);
	CExpression *pexprSubqueryQuantified3 = PexprSubqueryQuantified(memory_pool, eopid, pexprOuter1, pexprSelect2, fCorrelated);
	...
	)
{
	...
	switch (eopid)
	{
		...
		case COperator::EopScalarSubqueryAll:
			pexprSubquery = PexprSubqueryQuantified(memory_pool, eopid, pexprOuter, pexprInner, fCorrelated);
			break;

		case COperator::EopScalarSubqueryExists:
		case COperator::EopScalarSubqueryNotExists:
			pexprSubquery = PexprSubqueryExistential(memory_pool, eopid, pexprOuter, pexprInner, fCorrelated);
			break;
	...
	)
{
	...
	CExpression *pexprLogicalGbAgg = CUtils::PexprLogicalGbAggGlobal(memory_pool, pdrgpcr, pexprSelect, pexprProjList);
