...
CRangeTest::EresUnittest_CRangeFromScalar()
{
	...
	for (ULONG ul = 0; ul < GPOS_ARRAY_SIZE(rgecmpt); ul++)
	{
		...
		PrintRange(memory_pool, pcr, prange);
		prange->Release();
	}
	...
	)
{
	...
	PrintRange(memory_pool, pcr, prange1);
	PrintRange(memory_pool, pcr, prange2);
	PrintRange(memory_pool, pcr, prange3);
	PrintRange(memory_pool, pcr, prange4);
	PrintRange(memory_pool, pcr, prange5);
...
{
	CExpression *pexpr = prange->PexprScalar(memory_pool, pcr);
