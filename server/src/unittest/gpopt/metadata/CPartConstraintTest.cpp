...
CPartConstraintTest::EresUnittest_Basic()
{
	...
	CConstraint *pcnstr13 = PcnstrInterval(memory_pool, pcr, 1 /*ulLeft*/, 3 /*ulRight*/);
	
	// create a constraint col \in [1,5)
	CConstraint *pcnstr15 = PcnstrInterval(memory_pool, pcr, 1 /*ulLeft*/, 5 /*ulRight*/);
		
	CPartConstraint *ppartcnstr13Default = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pcnstr13, true /*fDefaultPartition*/, false /*fUnbounded*/);
	CPartConstraint *ppartcnstr15Default = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pcnstr15, true /*fDefaultPartition*/, false /*fUnbounded*/);

	pcnstr13->AddRef();
	CPartConstraint *ppartcnstr13NoDefault = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pcnstr13, false /*fDefaultPartition*/, false /*fUnbounded*/);

	pcnstr13->AddRef();
	CPartConstraint *ppartcnstr13DefaultUnbounded = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pcnstr13, true /*fDefaultPartition*/, true /*fUnbounded*/);

	pcnstr15->AddRef();
	CPartConstraint *ppartcnstr15DefaultUnbounded = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pcnstr15, true /*fDefaultPartition*/, true /*fUnbounded*/);
	...
	)
{
	...
	pdrgpexpr->Append(CUtils::PexprScalarCmp(memory_pool, pcr, pexprConstLeft, IMDType::EcmptGEq));
	pdrgpexpr->Append(CUtils::PexprScalarCmp(memory_pool, pcr, pexprConstRight, IMDType::EcmptL));

	CExpression *pexpr = CUtils::PexprScalarBoolOp(memory_pool, CScalarBoolOp::EboolopAnd, pdrgpexpr);
	CConstraint *pcnstr = CConstraintInterval::PciIntervalFromScalarExpr(memory_pool, pexpr, pcr);
...
CPartConstraintTest::EresUnittest_DateIntervals()
{
	...
	CPartConstraint *ppartcnstr1Default = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pciFirst, true /*fDefaultPartition*/, false /*fUnbounded*/);
	CPartConstraint *ppartcnstr2Default = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pciSecond, true /*fDefaultPartition*/, false /*fUnbounded*/);

	pciFirst->AddRef();
	CPartConstraint *ppartcnstr1NoDefault = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pciFirst, false /*fDefaultPartition*/, false /*fUnbounded*/);

	pciFirst->AddRef();
	CPartConstraint *ppartcnstr1DefaultUnbounded = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pciFirst, true /*fDefaultPartition*/, true /*fUnbounded*/);

	pciSecond->AddRef();
	CPartConstraint *ppartcnstr2DefaultUnbounded = GPOS_NEW(memory_pool) CPartConstraint(memory_pool, pciSecond, true /*fDefaultPartition*/, true /*fUnbounded*/);
